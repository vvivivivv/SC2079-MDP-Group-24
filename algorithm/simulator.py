...
# =============================================================================
# We use these global variables so we can change them with keys
CURRENT_FWD_RATIO = 0.38      # For FR, FL (Arrow Keys)
CURRENT_REV_RATIO = 0.50      # For BR, BL (W/S Keys)
CURRENT_SPEED = 30.0
...
class Robot:
    ...
    def reset(self):
        self.x = 15; self.y = 15; self.theta = 90
        self.is_running = False; self.commands = []
        self.path_history = [(15, 15)]
    ...
    def _decode_command(self, cmd):
        ...
        # --- FORWARD TURNS (Use FWD_RATIO) ---
        elif cmd.startswith("FR"): 
            self.target_val = 90; self.state = "TURNING"
            self.vl = CURRENT_SPEED
            self.vr = CURRENT_SPEED * CURRENT_FWD_RATIO # <--- FWD Variable

        elif cmd.startswith("FL"):
            self.target_val = 90; self.state = "TURNING"
            self.vl = CURRENT_SPEED * CURRENT_FWD_RATIO # <--- FWD Variable
            self.vr = CURRENT_SPEED

        # --- REVERSE TURNS (Use REV_RATIO) ---
        elif cmd.startswith("BR"): 
            self.target_val = 90; self.state = "TURNING"
            self.vl = -CURRENT_SPEED
            self.vr = -CURRENT_SPEED * CURRENT_REV_RATIO # <--- REV Variable

        elif cmd.startswith("BL"): 
            self.target_val = 90; self.state = "TURNING"
            self.vl = -CURRENT_SPEED * CURRENT_REV_RATIO # <--- REV Variable
            self.vr = -CURRENT_SPEED
...
# =============================================================================
# 5. MAIN LOOP (Updated with Controls)
# =============================================================================
def main():
    global CURRENT_FWD_RATIO, CURRENT_REV_RATIO, CURRENT_SPEED
    pygame.init()
    screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
    pygame.display.set_caption("MDP Simulator - LIVE TUNING MODE")
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # Fwd Ratio (Arrow Keys)
                if event.key == pygame.K_UP:
                    CURRENT_FWD_RATIO += 0.01
                elif event.key == pygame.K_DOWN:
                    CURRENT_FWD_RATIO -= 0.01
                
                # Rev Ratio (W/S Keys)
                elif event.key == pygame.K_w:
                    CURRENT_REV_RATIO += 0.01
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if start_btn.collidepoint(mouse_x, mouse_y):
                    # ... (API Request logic mostly same as before, simplified for brevity) ...
                    ...
                    robot.reset(); start_time = None; elapsed_time = 0.0
                    for ob in obstacles: ob.is_scanned = False
                elif reset_btn.collidepoint(mouse_x, mouse_y):
                    robot.reset(); obstacles.clear(); Obstacle._id_counter = 1
                    start_time = None; elapsed_time = 0.0
        ...
        robot.draw_path(screen); robot.draw(screen)
        ...
        # --- TUNING UI ---
        screen.blit(font.render("LIVE TUNING", True, BLUE), (ARENA_WIDTH + 10, 80))
        screen.blit(font.render(f"FWD Ratio (↑/↓): {CURRENT_FWD_RATIO:.2f}", True, BLACK), (ARENA_WIDTH + 10, 110))
        screen.blit(font.render(f"REV Ratio (W/S): {CURRENT_REV_RATIO:.2f}", True, BLACK), (ARENA_WIDTH + 10, 140))
        ...
        pygame.draw.rect(screen, GREEN if len(obstacles)>=1 else LIGHT_GRAY, start_btn)
        pygame.draw.rect(screen, BLACK, start_btn, 2)
        screen.blit(font.render("START", True, BLACK), (ARENA_WIDTH + 60, 460))
