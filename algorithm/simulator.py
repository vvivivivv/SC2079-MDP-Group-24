...
#
TURN_DISPLACEMENT = {
    "FR": ( -30,  15),   # Forward Right:  turns 90° CW
    "FL": (30,  15),   # Forward Left:   turns 90° CCW
    "BR": (-15, -30),   # Backward Right: turns 90° CCW
    "BL": ( 15, -30),   # Backward Left:  turns 90° CW
}
...
class Robot:
    ...
    def update(self, current_scanned_ids=None):
        ...
        # 3. Handle DISP_TURN — animated displacement turn (90°)
        ...
        # 4. PHYSICS: STRAIGHT-LINE MOVEMENT
    ...
    def _decode_command(self, cmd):
        ...
        # 1. DISPLACEMENT TURNS (FL, FR, BL, BR) — always 90°, configurable (dx, dy)
        if cmd[:2] in ["FR", "FL", "BR", "BL"]:
            ...
            # Determine rotation direction: FR/BL = -90° (CW), FL/BR = +90° (CCW)
            if turn_type in ["FR", "BL"]:
                angle_change = -90
            else:
                angle_change = 90
        ...
        # 2. STRAIGHT MOVES (FW, BW) — distances in mm
