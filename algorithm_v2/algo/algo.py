...
Architecture:
  1. Get candidate capture positions for each obstacle (from Entity.py)
  2. Build TSP cost matrix using DUBINS PATH LENGTH (not grid Manhattan distance)
  3. Solve TSP with python_tsp for optimal visit order
  4. Plan each leg with Hybrid A* (Dubins heuristic, respects turning radius)
  5. Convert paths to TL/TR/FW/BW motor commands

No grid A* anywhere — all distances are real Dubins curves.
"""
...
class MazeSolver:
    ...
    def _state_to_pose(state: CellState):
        ...
        We detect which by checking if theta_rad is set (view cone candidates
        always set it).
        """
        if hasattr(state, 'theta_rad') and state.theta_rad is not None:
            # View cone candidate: x,y are continuous grid units
            x_cm = state.x * 10
            y_cm = state.y * 10
            theta = state.theta_rad
        else:
            # Integer grid cell (e.g., start state): center is at x*10 + 5
            x_cm = state.x * 10 + 5
            y_cm = state.y * 10 + 5
    ...
    # =================================================================
    # DUBINS DISTANCE (replaces grid A* for TSP cost matrix)
    # =================================================================

    @staticmethod
    def _dubins_distance(pose_a, pose_b):
        """Compute Dubins path length between two poses (no obstacle check)."""
        result = compute_dubins_path(pose_a, pose_b, TURN_RADIUS_CM)
    ...
    @staticmethod
    def _sample_dubins_path(pose_a, pose_b):
        """Sample points along a Dubins path for collision checking."""
        result = compute_dubins_path(pose_a, pose_b, TURN_RADIUS_CM)
        if result is None:
    ...
    def _dubins_distance_obstacle_aware(self, pose_a, pose_b, obstacle_centers,
                                         skip_ids=None):
        """Dubins distance with penalty for paths crossing obstacles.
        
        Uses a fast straight-line pre-check: only does expensive path sampling
        if the straight line passes within (OBSTACLE_RADIUS + turning_diameter) 
        of an obstacle.
        """
        base_dist = self._dubins_distance(pose_a, pose_b)
        ...
        # Threshold: obstacle radius + max Dubins detour from straight line
        # (Dubins path can deviate up to ~2*R from the straight line)
        ...
        # Detailed check: sample along actual Dubins path
        points = self._sample_dubins_path(pose_a, pose_b)
    ...
    def get_optimal_order_dp(self, retrying) -> Tuple[List[CellState], float]:
        """Find optimal obstacle visit order using Dubins-based TSP.
        
        Returns:
            (optimal_path, distance) where optimal_path is a list of CellStates
            representing the visit order and capture positions.
        """
        ...
        for op in self._get_visit_options(len(all_view_positions)):
            # Build list of active obstacles for this combination
            items_states = [start_state]  # index 0 = robot start
            ...
            # Step 2: Pre-compute ALL pairwise obstacle-aware Dubins distances
            n_total = len(items_poses)
            
            # Build obstacle ID for each pose
            all_ob_ids = []
            ...
            for s in range(n_total):
                for e in range(n_total):
                    ...
                    full_cost[s][e] = self._dubins_distance_obstacle_aware(
            ...
            for c in combinations:
                # Select one candidate per obstacle
                visited_indices = [0]  # start at robot position
                cur_index = 1
                fixed_cost = 0
                ...
                # Extract sub-matrix from pre-computed costs
                ...
                # Record best: build path as ordered CellStates
                best_distance = _distance + fixed_cost
                best_path = [items_states[0]]  # start

                for i in range(1, len(_permutation)):
                    if _permutation[i] == 0:
                        continue  # skip return to start
                    state = items_states[visited_indices[_permutation[i]]]
                    best_path.append(state)

            if best_path:
                break  # found a valid path with all obstacles
    ...
    def plan_full_route(self, retrying=False, obstacles_data=None):
        ...
        Args:
            retrying: whether to use extended capture range
            obstacles_data: list of obstacle dicts {'x','y','d','id'}
        
        Returns:
            (commands, waypoint_path, distance)
            commands: list of motor command strings
            waypoint_path: list of CellStates (visit order)
            distance: total Dubins distance
        """
        t0 = time.time()

        # Phase 1: Find optimal visit order using Dubins TSP
        waypoint_path, distance = self.get_optimal_order_dp(retrying)
        t_tsp = time.time() - t0
        print(f"Phase 1 (Dubins TSP): {t_tsp:.2f}s, distance={distance:.0f}cm")
        ...
        # Track the ACTUAL robot pose (chains legs together)
        ...
        for i in range(len(waypoint_path) - 1):
            ...
            # Start from where the robot ACTUALLY is (not the planned waypoint)
            start_pose = current_pose
            goal_pose = self._state_to_pose(wp_to)
            
            # Determine which obstacles are the start/goal targets
            # so Hybrid A* can relax their collision radius for approach
            start_ob_idx = None
            goal_ob_idx = None
            ...
            # Run Hybrid A* with Dubins heuristic
            ...
            if path:
                ...
                # Update current_pose to where Hybrid A* ACTUALLY ended
                end = path[-1]
                current_pose = (end[0], end[1], end[2])
                
                print(f"  Leg {i}: {len(path)} steps, {seg_dist:.0f}cm, {iters} iters [{moves}]")
            else:
                # Hybrid A* failed — retry with relaxed tolerance
            ...
            if wp_to.screenshot_id != -1 and obstacles_dict:
                ...
                # Just emit SNAP — no unchecked correction moves.
                # The A* path + view cone geometry ensures the face is within
                # the camera's 62° FOV (±31°). The signal tells the robot
        ...
        print(f"Phase 2 (Hybrid A*): {t_hybrid:.2f}s")
