...
class Robot:
    ...
    def _decode_command(self, cmd):
        ...
        # 1. ARC MOVES (FL, FR, BL, BR) — command values in mm
        if cmd[:2] in ["FR", "FL", "BR", "BL"]:
            try:
                ...
                self.target_val = dist_mm / 10.0  # convert mm -> cm for physics
                self.state = "MOVING"
        ...
        # 2. STRAIGHT MOVES (FW, BW) — command values in mm
        elif cmd.startswith("FW"):
            try: val = int(cmd[2:])
            except: val = 100
            self.target_val = val / 10.0; self.state = "MOVING"  # mm -> cm
            self.velocity = CURRENT_SPEED
            self.steering_angle = 0.0
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if setpos_btn.collidepoint(mouse_x, mouse_y):
                    ...
                    root = tkinter.Tk()
                    root.withdraw()
                    cmd_input = simpledialog.askstring("Manual Command", "Enter Command (e.g. FW200, FL390, SNAP1):")
                    root.destroy()
