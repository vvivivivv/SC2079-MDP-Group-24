...
def rs_segments_to_commands(segments):
    ...
    Long arc segments are broken into smaller chunks (max MAX_ARC_CMD_CM)
    because the real robot's arc accuracy degrades over distance — motor
    slop, wheel slip, and friction accumulate.  Short arcs keep each
    individual command's error small and prevent compounding.
    """
    MAX_ARC_CMD_CM = 20  # max cm per single arc command
    ...
    for seg_type, length_cm, gear in segments:
        ...
        if seg_type == 'S':
            ...
            remaining = dist
            while remaining > 0:
                chunk = min(remaining, MAX_ARC_CMD_CM)
