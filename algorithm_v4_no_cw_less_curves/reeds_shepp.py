...
def sample_path(start, radius, end=None, segments=None, num_points=50,
    ...
    Args:
        ...
        radius: turning radius in cm (default for all arcs)
        ...
        segment_radii: optional list of per-segment radii (same length as segments).
                       If provided, each arc segment uses its own actual radius
                       for sampling, giving the true trajectory the robot will follow.
    ...
    for seg_idx, (seg_type, length, gear) in enumerate(segments):
        if length < 1e-6:
            continue

        # Use per-segment radius if provided, otherwise default
        R = segment_radii[seg_idx] if segment_radii else radius
