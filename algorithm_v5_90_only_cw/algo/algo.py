...
from consts import Direction, MOVE_DIRECTION, TURN_FACTOR, ITERATIONS, SAFE_COST
from python_tsp.exact import solve_tsp_dynamic_programming

# Half-track in grid units (half the axle track / cell size)
# e.g. axle track = 16cm, cell = 10cm → h = 0.8 → round to 1
PIVOT_HALF_TRACK = 1

# Pivot turn displacement tables: (from_dir, to_dir) → (dx, dy)
# Left-wheel pivot: center swings around the left wheel
PIVOT_LEFT = {
    (Direction.NORTH, Direction.EAST):  (-PIVOT_HALF_TRACK, -PIVOT_HALF_TRACK),
    (Direction.NORTH, Direction.WEST):  (-PIVOT_HALF_TRACK,  PIVOT_HALF_TRACK),
    (Direction.EAST,  Direction.SOUTH): (-PIVOT_HALF_TRACK,  PIVOT_HALF_TRACK),
    (Direction.EAST,  Direction.NORTH): ( PIVOT_HALF_TRACK,  PIVOT_HALF_TRACK),
    (Direction.SOUTH, Direction.WEST):  ( PIVOT_HALF_TRACK,  PIVOT_HALF_TRACK),
    (Direction.SOUTH, Direction.EAST):  ( PIVOT_HALF_TRACK, -PIVOT_HALF_TRACK),
    (Direction.WEST,  Direction.NORTH): ( PIVOT_HALF_TRACK, -PIVOT_HALF_TRACK),
    (Direction.WEST,  Direction.SOUTH): (-PIVOT_HALF_TRACK, -PIVOT_HALF_TRACK),
}

# Right-wheel pivot: center swings around the right wheel
# Displacements are the exact opposite of left-wheel pivots
PIVOT_RIGHT = {
    (Direction.NORTH, Direction.EAST):  ( PIVOT_HALF_TRACK,  PIVOT_HALF_TRACK),
    (Direction.NORTH, Direction.WEST):  ( PIVOT_HALF_TRACK, -PIVOT_HALF_TRACK),
    (Direction.EAST,  Direction.SOUTH): ( PIVOT_HALF_TRACK, -PIVOT_HALF_TRACK),
...
class MazeSolver:
    ...
    def get_neighbors(self, x, y, direction):  # TODO: see the behavior of the robot and adjust...
        ...
        for dx, dy, md in MOVE_DIRECTION:
            ...
            else:  # Pivot turn — try both left-wheel and right-wheel pivots
                key = (direction, md)
                for pivot_table in (PIVOT_LEFT, PIVOT_RIGHT):
                    if key in pivot_table:
                        pdx, pdy = pivot_table[key]
                        nx, ny = x + pdx, y + pdy
                        if self.grid.reachable(nx, ny, turn=True) and self.grid.reachable(x, y, preTurn=True):
                            safe_cost = self.get_safe_cost(nx, ny)
                            neighbors.append((nx, ny, md, safe_cost + 10))
