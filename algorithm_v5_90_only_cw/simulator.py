...
ROBOT_SPEED_CM_S = 30
ROBOT_AXLE_TRACK_CM = 16
ROBOT_WHEELBASE_CM = 20
SCALE_FW = 1
SCALE_BW = 1
OFFSET_FW = 0
OFFSET_BW = 0
# =============================================================================
# CONFIGURABLE TURN DISPLACEMENTS (cm, measured from robot center)
# =============================================================================
# Each entry: (dx_cm, dy_cm) where:
#   dx = displacement along the robot's LOCAL LEFT/RIGHT axis
#         positive = right, negative = left
#   dy = displacement along the robot's LOCAL FORWARD/BACKWARD axis
#         positive = forward, negative = backward
#
# Adjust these values to match your real robot's measured turn behavior.
# The robot always rotates exactly 90 degrees for any turn command.
# h = half axle track (8cm for 16cm track)
#
# Left-wheel pivots: center shifts LEFT
#   FR (CW on left wheel):  left + backward
#   FL (CCW on left wheel): left + forward
# Right-wheel pivots: center shifts RIGHT
#   BR (CW on right wheel):  right + forward
#   BL (CCW on right wheel): right + backward
#
TURN_DISPLACEMENT = {
    "FR": (-8.0, -8.0),   # left-wheel CW:   left + backward
    "FL": (-8.0,  8.0),   # left-wheel CCW:  left + forward
    "BR": ( 8.0,  8.0),   # right-wheel CW:  right + forward
    "BL": ( 8.0, -8.0),   # right-wheel CCW: right + backward
}

# Duration of the animated turn in seconds
TURN_ANIM_DURATION = 0.5
...
class Robot:
    def __init__(self, x, y, theta):
        ...
        # Displacement turn animation state
        self.turn_start_x = 0.0
        self.turn_start_y = 0.0
        self.turn_start_theta = 0.0
        self.turn_target_x = 0.0
        self.turn_target_y = 0.0
        self.turn_target_theta = 0.0
        self.turn_elapsed = 0.0
        self.turn_duration = TURN_ANIM_DURATION

        # Visual assets — trapezoid robot body inside 30x30 bounding box.
        # Surface coords: +x = forward, +y = down (right side of robot).
        # Robot center = surface center = (half, half).
        scale = ARENA_WIDTH / GRID_SIZE
        size_px = int(ROBOT_SIZE_CM * scale)
        half = size_px / 2  # center in pixels (= 15cm * scale)

        # Trapezoid corners in cm relative to center, then convert to px.
        # Forward axis (x): side length 23cm, centered → ±11.5cm
        # Front edge (x=+11.5): 18.6cm wide → ±9.3cm laterally
        # Back edge  (x=-11.5): 18.8cm wide → ±9.4cm laterally
        sx = ROBOT_SIDE_LENGTH_CM / 2   # 11.5
        fw = ROBOT_FRONT_WIDTH_CM / 2   # 9.3
        bw = ROBOT_BACK_WIDTH_CM / 2    # 9.4

        # (surface_x, surface_y) = (center + forward_cm * scale, center + lateral_cm * scale)
        trap_pts = [
        ...
        # Heading line from center to front edge center
    ...
    def save_snapshot(self, scanned_ids):
        """Saves current physical state BEFORE executing a new command"""
    ...
    def restore_snapshot(self):
        """Pops the stack and reverts robot to the previous state"""
    ...
    def _compute_pivot_point(self):
        """Compute the back-left wheel position in world frame."""
    ...
    def _rotate_around_pivot(self, delta_deg):
        """Rotate robot center around the fixed pivot point (back-left wheel)."""
        alpha = math.radians(delta_deg)  # CW rotation angle
    ...
    def update(self, current_scanned_ids=None):
        ...
        if self.state == "IDLE":
            ...
            if self.step_mode and not self.step_next:
                return None  # Pause execution and wait for user to click FWD
            
            self.step_next = False  # Consume the step signal

            # Snapshot state BEFORE decoding the new command
        ...
        # 3b. Handle DISP_TURN — animated displacement turn (90°)
        if self.state == "DISP_TURN":
            self.turn_elapsed += dt
            t = min(self.turn_elapsed / self.turn_duration, 1.0)
            # Smooth interpolation (ease in-out)
            t_smooth = t * t * (3 - 2 * t)

            self.x = self.turn_start_x + (self.turn_target_x - self.turn_start_x) * t_smooth
            self.y = self.turn_start_y + (self.turn_target_y - self.turn_start_y) * t_smooth
            self.theta = self.turn_start_theta + (self.turn_target_theta - self.turn_start_theta) * t_smooth
            self.path_history.append((self.x, self.y))

            if t >= 1.0:
                self.x = self.turn_target_x
                self.y = self.turn_target_y
                self.theta = self.turn_target_theta % 360
                self.state = "IDLE"
                self.current_cmd_idx += 1
    ...
    def _decode_command(self, cmd):
        ...
        # 1. PIVOT TURNS — always 90°, configurable displacement
        #    FR/BR = CW (-90°), FL/BL = CCW (+90°)
        if cmd[:2] in ["FR", "FL", "BR", "BL"]:
            turn_type = cmd[:2]
            local_dx, local_dy = TURN_DISPLACEMENT[turn_type]

            # Convert local displacement to world frame using current heading
            theta_rad = math.radians(self.theta)
            cos_t = math.cos(theta_rad)
            sin_t = math.sin(theta_rad)

            # local_dy = forward axis, local_dx = right axis
            world_dx = local_dy * cos_t - local_dx * sin_t
            world_dy = local_dy * sin_t + local_dx * cos_t

            # FR/BR = CW = -90°, FL/BL = CCW = +90°
            angle_change = -90 if turn_type in ["FR", "BR"] else 90

            # Set up animated turn
            self.turn_start_x = self.x
            self.turn_start_y = self.y
            self.turn_start_theta = self.theta
            self.turn_target_x = self.x + world_dx
            self.turn_target_y = self.y + world_dy
            self.turn_target_theta = self.theta + angle_change
            self.turn_elapsed = 0.0
            self.state = "DISP_TURN"
            return
