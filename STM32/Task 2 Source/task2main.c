...
#define KP_DIFF        (40.0f * 60.0f)//(25.0f * 60.0f)    // proportional [PWM per RPS]
#define KI_DIFF        (1.0f * 60.0f) //(0.6f * 60.0f)     // integral [PWM per RPS per second]
#define KD_DIFF        (0.00f * 60.0f)   // derivative [PWM per RPS * second] (start at 0)

#define I_MAX          4000.0f  // integral clamp (PWM units)
#define PWM_MIN_CLAMP  4500
...
static const float GYRO_SENS_LSB_PER_DPS = 134.0f; //131.0f;  // FS=±250/500/1000/2000dps -> adjust if needed
static const float GYRO_LP_ALPHA = 0.67f;   // IIR LPF coefficient

/* ================= Group 24 Controls Config ================= */
int servoOffset = 0; // 0 if last turn was left, -8 if last turn was right.
int slowdownNow = 0;
int sonicCount = 0;
int irCount = 0;
int irSlowdown = 0;
/* ================= Steer control config ================= */
#define STEER_US_LEFT      1300    // +36°
#define STEER_US_CENTER    1755    //  0°
#define STEER_US_RIGHT     2200    // -36°
...
#define YAW_COARSE_THRESH_DEG  10.0f
#define YAW_FINE_DB_DEG        1.0f // tolerance to complete turn
#define YAW_STOP_DB_DEG        0.5f

#define TURN_PWM_MAX           5250
#define TURN_PWM_MIN           4500
#define MICRO_NUDGE_PWM        4250
#define MICRO_NUDGE_MS         10

#define FINE_KP_STEER          0.85f
#define FINE_KD_RATE           0.10f
...
/* --- Command Cooldown --- */
#define CMD_COOLDOWN_MS   200u //10u   // tweak 150–500 ms as you like
...
static inline int cmdq_empty(void) {
...
const osThreadAttr_t IMUTask_attributes = { .name = "IMUTask", .stack_size =
		1024 * 4, .priority = (osPriority_t) osPriorityNormal, };
/* Definitions for ServoMotorTask */
osThreadId_t ServoMotorTaskHandle;
const osThreadAttr_t ServoMotorTask_attributes = { .name = "ServoMotorTask",
		.stack_size = 256 * 4, .priority = (osPriority_t) osPriorityLow, };
/* Definitions for IRTask */
osThreadId_t IRTaskHandle;
const osThreadAttr_t IRTask_attributes = { .name = "IRTask", .stack_size = 256
...
static HAL_StatusTypeDef icm_read_gyro_xyz(int16_t *gx, int16_t *gy,
		int16_t *gz);

/* Helper to drive a dual-input H-bridge using two PWM channels */
static inline void Motor_Set(TIM_HandleTypeDef *htim, uint32_t ch_fwd,
		uint32_t ch_rev, uint16_t duty, uint8_t dir) {
	/* dir: 0 = CW, 1 = CCW */
	if (dir == 0) {
...
static inline int Servo_RequestTurnTo(float target_heading_deg) {
	if (g_steer_cmd.busy)
		return 1; // still handling previous command
...
#define E_BRAKE_PWM   5000 //4500   // tweak 4000–5500
#define E_BRAKE_MS    40 //80     // tweak 60–120 ms
...
static inline void ActiveBrakeStopForward(void) {
	...
	if (ir_left || ir_right) {
		Motor_Set(&htim4, TIM_CHANNEL_3, TIM_CHANNEL_4, E_BRAKE_PWM + 1000, 0); // A reverse of forward
		Motor_Set(&htim1, TIM_CHANNEL_3, TIM_CHANNEL_4, E_BRAKE_PWM + 1000, 1); // D reverse of forward
		osDelay(E_BRAKE_MS + 20);
	}
...
int32_t IrConvert(int32_t nc) {
	if (nc < 1)
		nc = 1;      // avoid divide by zero / powf domain errors
	if (nc > 4095)
		nc = 4095;   // ADC max
	...
	if (d_cm < 0.0f)
		d_cm = 0.0f;   // no negative distance
...
int main(void) {
	...
	ServoMotorTaskHandle = osThreadNew(servomotor, NULL,
			&ServoMotorTask_attributes);

	/* creation of IRTask */
	IRTaskHandle = osThreadNew(ir, NULL, &IRTask_attributes);

	/* creation of UltrasonicTask */
	UltrasonicTaskHandle = osThreadNew(ultrasonic, NULL,
...
static void MX_GPIO_Init(void) {
	...
	HAL_GPIO_WritePin(GPIOD,
			OLED_DC_Pin | OLED_RST_Pin | OLED_SDA_Pin | OLED_SCL_Pin,
			GPIO_PIN_RESET);
...
static int icm_probe(void) {
	...
	for (int i = 0; i < 2; ++i) {
		...
		if (icm_read(addrs[i], ICM_REG_WHO_AM_I, &who) == HAL_OK
				&& who == 0xEA) {
			ICM_addr = addrs[i];
			ICM_whoami = who;
			return 1;
...
static HAL_StatusTypeDef icm_read_gyro_xyz(int16_t *gx, int16_t *gy,
	...
	if (icm_select_bank(ICM_addr, 0) != HAL_OK)
		return HAL_ERROR;
...
static inline uint16_t steer_deg_to_pulse(float wheel_deg) {
	if (wheel_deg > STEER_MAX_DEG)
		wheel_deg = STEER_MAX_DEG;
	if (wheel_deg < -STEER_MAX_DEG)
		wheel_deg = -STEER_MAX_DEG;
	/* 1500 - 16.6667*deg  (since +36° => 900us, -36° => 2100us) */
//  float usec = 1500 - (wheel_deg * (600.0f/36.0f));
	float usec = 0;

	//Map independently for Left vs Right to account for mechanical asymmetry
	if (wheel_deg >= 0.0f) {
		// Turning LEFT: Map 0 to +36 degrees -> 1800 down to 1250
		usec = STEER_US_CENTER
				+ wheel_deg
						* ((STEER_US_LEFT - STEER_US_CENTER) / STEER_MAX_DEG);
	} else {
		// Turning RIGHT: Map 0 to -36 degrees -> 1800 up to 2300
		// (wheel_deg is negative here, so we divide by -STEER_MAX_DEG)
		usec = STEER_US_CENTER
				+ wheel_deg
						* ((STEER_US_RIGHT - STEER_US_CENTER) / -STEER_MAX_DEG);
	}

	if (usec < STEER_US_LEFT)
		usec = STEER_US_LEFT;
	if (usec > STEER_US_RIGHT)
		usec = STEER_US_RIGHT;
...
static void steer_center(void) {
	steer_write_us(STEER_US_CENTER + servoOffset);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart->Instance == USART3) {
		...
		if (ch == '\n' || ch == '\r') {
			if (uart3_idx >= sizeof(uart3_line))
				uart3_idx = sizeof(uart3_line) - 1;
			...
			if (cmd[0] != '\0') {
				if (cmdq_push(cmd) != 0) {
					...
					HAL_UART_Transmit(&huart3, (uint8_t*) busy,
							(uint16_t) strlen(busy), 0xFFFF);
				}
			}
			uart3_idx = 0;
		} else {
...
static void CommandQueue_TryDispatch(void) {
	if (motionActive || g_steer_cmd.busy)
		return;

	// NEW: respect cooldown window
	if (HAL_GetTick() < g_cmd_cooldown_until_ms)
		return;
	...
	char cmdline[32];
	if (cmdq_pop(cmdline, sizeof cmdline) != 0)
		return;
	...
	if (c0 == 'Y' && c1 == 'F') {
		...
		if (sonicCount == 3) {
			if (d <= 15.0f) {
				...
				if (to_go < 0.0f)
					to_go = 0.0f;
				if (to_go >= 10.0f) {
					if(to_go >= 25.0f) to_go = 25.0f;
					StartMoveCM(to_go, DIR_FWD);
				}
			}
			int after_move = 1;
			char line[16];
			...
			if (n > 0 && n < (int) sizeof(line)) {
				HAL_UART_Transmit(&huart3, (uint8_t*) line, (uint16_t) n,
						HAL_MAX_DELAY);
			}
			return;
		}
		...
		if (d <= 35.0f) {
			// 1) back up 35 cm
			int goback = (int) (d - 35.f);
			goback = fabs(goback);
			StartMoveCM(goback, DIR_BACK);

			// 2) then send (d + 35) as an integer
		...
		float to_go = d - 35.0f;  // stop ~35 cm with 2 cm margin
		if (to_go < 0.0f)
			to_go = 0.0f;
		...
		if (to_go >= 10.0f)
			StartMoveCM((int) (to_go + 0.5f), DIR_FWD);
		else
			uart3_send("YF40\r\n");
		return;
	}
	...
	if (c0 == 'F' && c1 == 'L' && isdigit((unsigned char )s[2])) {
		...
		if (Servo_RequestBangBangTurn(+(float) deg, 4500) == 0)
			uart3_send("A\r\n");
		return;
	}
	if (c0 == 'F' && c1 == 'R' && isdigit((unsigned char )s[2])) {
	...
	// ABS
	if (c0 == 'A' && c1 == 'B' && toupper((unsigned char)s[2]) == 'S') {
		int absd = atoi(&s[3]) % 360;
		if (Servo_RequestTurnTo((float) absd) == 0)
			uart3_send("A\r\n");
		else
			uart3_send("BUSY\r\n");
		return;
	...
	if (c0 == 'F' && c1 == 'W') {
		int cm = 0;
		if (sscanf(p, "%d", &cm) == 1 && cm > 0) {
			StartMoveCM(cm, DIR_FWD);
			uart3_send("A\r\n");
		} else
			uart3_send("ERR (use FW###)\r\n");
		return;
	}
	if (c0 == 'B' && c1 == 'W') {
		int cm = 0;
		if (sscanf(p, "%d", &cm) == 1 && cm > 0) {
			StartMoveCM(cm, DIR_BACK);
			uart3_send("A\r\n");
		} else
			uart3_send("ERR (use BW###)\r\n");
		return;
	}
...
void motor(void *argument) {
	...
	const int pwmBase = 5500; // your feed-forward
	...
	for (;;) {
		...
		// clamp
		if (pwmA_val < PWM_MIN_CLAMP)
			pwmA_val = PWM_MIN_CLAMP;
		if (pwmA_val > PWM_MAX_CLAMP)
			pwmA_val = PWM_MAX_CLAMP;
		if (pwmD_val < PWM_MIN_CLAMP)
			pwmD_val = PWM_MIN_CLAMP;
		if (pwmD_val > PWM_MAX_CLAMP)
			pwmD_val = PWM_MAX_CLAMP;
		...
		if (motionActive) {
			...
			if (dir == DIR_FWD) {
				...
				if (cp1 == 0) {
					start_angle = yaw_angle_deg;
					cp1 = 1;
				}
				//char msg[64]; int n=sprintf(msg,"hi! %.1f %.1f\r\n",yaw_angle_deg, start_angle);
				//HAL_UART_Transmit_IT(&huart3,(uint8_t*)msg,(uint16_t)n);
				//uart3_send(msg);
				if (yaw_angle_deg < start_angle) { // drifted right
					steer_write_us(STEER_US_CENTER - 80);
				}
				if (yaw_angle_deg > start_angle) { // drifted left
					steer_write_us(STEER_US_CENTER + 80);
				}
...
void distance(void *argument) {
	...
	for (;;) {
		...
		float slowdownTest = (float) targetdistance_cm - avg;
		if (slowdownTest <= 15 && !ir_left && !ir_right) {
			slowdownNow = 1;
		}

		if (motionActive && avg >= (float) targetdistance_cm && !ir_left
				&& !ir_right) {
			AllStop();
			motionActive = 0;
			targetdistance_cm = 0;
			...
			if (dir == DIR_FWD) {
				ActiveBrakeStopForward();
			} else {
				ActiveBrakeStopBackward();
			}
...
void imu(void *argument) {
	...
	for (;;) {
		if (icm_read_gyro_xyz(&gx, &gy, &gz) == HAL_OK) {
			...
			if (z_dps > -0.5f && z_dps < 0.5f) {
				z_dps = 0.0f;
			}

			yaw_rate_dps = yaw_rate_dps * (1.0f - GYRO_LP_ALPHA)
					+ z_dps * GYRO_LP_ALPHA;
...
void servomotor(void *argument) {
	...
	for (;;) {
		...
		const uint8_t rev_drive = g_steer_cmd.reverse_drive;
		...
		for (;;) {
			...
			if (abs_err > YAW_COARSE_THRESH_DEG) {
				...
				float wheel_cmd_deg = FINE_KP_STEER * err
						- FINE_KD_RATE * yaw_rate_dps;
			...
			if (abs_err <= YAW_STOP_DB_DEG) {
				...
				steer_center();
				osDelay(30);
				float check_err = fabsf(
						smallest_err_deg(target, yaw_angle_deg));
	...
	for (;;) {
		...
		if (ir_left) {
			...
			if (irCount == 2) {
				...
				if ((dL_cm >= 45 || dL_cm == 0)) {
					motionActive = 1;
					while (dL_cm >= 45 || dL_cm == 0) {
						for (int i = 0; i < 9; i++)
							bufL[i] = adc1_read_once();
						rawL = median9(bufL);
						dL_cm = IrConvert((int32_t) rawL);
					}
				}
			}
			osDelay(25);
			...
			if (dL_cm >= 45 || dL_cm == 0) {
				...
				ActiveBrakeStopForward();
				taskENTER_CRITICAL();
				ir_start_avg = (int) (fabsf(distance_cm_A)
						+ fabsf(distance_cm_D) + 54.0f) / 4;
				taskEXIT_CRITICAL();
				char line[16];
		...
		if (ir_right) {
			...
			if (irCount == 2) {
				...
				if ((dR_cm >= 45 || dR_cm == 0)) {
					motionActive = 1;
					while (dR_cm >= 45 || dR_cm == 0) {
						for (int i = 0; i < 9; i++)
							bufR[i] = adc1_read_once();
						rawR = median9(bufR);
						dR_cm = IrConvert((int32_t) rawR);
					}
				}
			}
